...
export default function ReportsPage({ darkMode }) {
  ...
  const [summaryRange, setSummaryRange] = useState('today'); // ✅ time range
  ...
  // ✅ Summary by time range calculations
  const getRangeStart = (range) => {
    ...
    if (range === 'month') { const d = new Date(now); d.setDate(d.getDate()-30);   d.setHours(0,0,0,0); return d; }
  ...
  const rangeTx       = transactions.filter(t => new Date(t.created_at) >= rangeStart);
  const rangeInTx     = rangeTx.filter(t => t.type === 'in');
  const rangeOutTx    = rangeTx.filter(t => t.type === 'out');
  const rangeInCost   = rangeInTx.reduce((s,t)  => s + parseFloat(t.total_price||0), 0);
  const rangeOutRev   = rangeOutTx.reduce((s,t) => s + parseFloat(t.total_price||0), 0);
  const rangeProfit   = rangeOutRev - rangeInCost;
  const rangeProducts = [...new Set(rangeTx.map(t => t.product_id))].length;
  const rangeQtyIn    = rangeInTx.reduce((s,t)  => s + (parseInt(t.quantity)||0), 0);
  const rangeQtyOut   = rangeOutTx.reduce((s,t) => s + (parseInt(t.quantity)||0), 0);
  ...
  const printInvoice = (inv) => {
    ...
    <style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Segoe UI',Arial,sans-serif;background:#f8fafc;color:#0f172a}
    .page{background:#fff;max-width:720px;margin:0 auto;box-shadow:0 4px 40px rgba(0,0,0,0.10);min-height:100vh}
    .header{background:linear-gradient(135deg,#14b8a6,#6366f1);padding:36px 44px 28px;color:#fff;display:flex;justify-content:space-between;align-items:flex-start}
    ...
    .info-box{padding:24px 44px}.info-box:first-child{border-right:1px solid #e2e8f0}
    .info-label{font-size:9px;font-weight:700;color:#14b8a6;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px}
    ...
    tbody td{padding:14px;border-bottom:1px solid #f1f5f9;font-size:13px}
    ...
    @media print{body{background:#fff}.page{box-shadow:none;max-width:100%}}</style></head>
    <body><div class="page">
    <div class="header">
      <div style="display:flex;align-items:center;gap:12px">
        <div class="brand-icon">📦</div>
        <div><div style="font-size:22px;font-weight:800">StockPro</div><div style="font-size:11px;opacity:0.75">Management System</div></div>
      </div>
      <div style="text-align:right">
        <div style="font-size:32px;font-weight:300;letter-spacing:4px;opacity:0.9">Invoice</div>
        <div class="inv-num">${invoiceNo}</div>
        <div style="font-size:11px;opacity:0.75;margin-top:6px">📅 ${new Date(inv.created_at).toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}</div>
      </div>
    </div>
    <div style="padding:14px 44px;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;gap:10px;background:${inv.type==='in'?'#f0fdf4':'#fff5f5'}">
      <span style="padding:5px 16px;border-radius:20px;font-size:11px;font-weight:700;background:${inv.type==='in'?'#dcfce7':'#fee2e2'};color:${inv.type==='in'?'#15803d':'#b91c1c'};border:1px solid ${inv.type==='in'?'#86efac':'#fca5a5'}">
        ${inv.type==='in'?'▼ STOCK IN':'▲ STOCK OUT'}
      </span>
      <span style="font-size:12px;color:#64748b">Recorded on ${new Date(inv.created_at).toLocaleString()}</span>
    </div>
    <div class="info-grid">
      <div class="info-box">
        <div class="info-label">📋 Invoice Details</div>
        <div style="font-size:12px;display:flex;flex-direction:column;gap:5px">
          <div><span style="color:#64748b;width:90px;display:inline-block">Invoice No:</span><span style="font-weight:700;font-family:monospace;color:#14b8a6">${invoiceNo}</span></div>
          <div><span style="color:#64748b;width:90px;display:inline-block">Date:</span><span style="font-weight:600">${new Date(inv.created_at).toLocaleDateString()}</span></div>
          <div><span style="color:#64748b;width:90px;display:inline-block">Type:</span><span style="font-weight:700;color:${inv.type==='in'?'#10b981':'#ef4444'}">${inv.type.toUpperCase()}</span></div>
          <div><span style="color:#64748b;width:90px;display:inline-block">By:</span><span style="font-weight:600">${user.name||'Admin'}</span></div>
        </div>
      </div>
      <div class="info-box">
        <div class="info-label">🏢 Company</div>
        <div style="font-size:16px;font-weight:700;margin-bottom:4px">StockPro Co., Ltd.</div>
        <div style="font-size:12px;color:#64748b;line-height:1.8">Stock Management System<br>📧 admin@stockpro.com<br>📞 +855 12 345 678</div>
      </div>
    </div>
    <div class="product-section">
      ${productImg?`<img src="${productImg}" style="width:70px;height:70px;border-radius:14px;object-fit:cover;border:2px solid #e2e8f0" />` : `<div class="product-placeholder">${(inv.product_name||'??').slice(0,2).toUpperCase()}</div>`}
      <div><div style="font-size:16px;font-weight:700">${inv.product_name}</div><div style="font-size:12px;color:#64748b;margin-top:4px">PRD-${String(inv.product_id||0).padStart(3,'0')}</div></div>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td><div style="font-weight:600">${inv.product_name}</div><div style="font-size:11px;color:#94a3b8;margin-top:2px">${inv.type==='in'?'Stock received into inventory':'Stock issued from inventory'}</div></td>
            ...
            <td style="text-align:right;font-family:monospace;font-weight:800;color:${inv.type==='in'?'#ef4444':'#10b981'}">$${parseFloat(inv.total_price||0).toFixed(2)}</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="total-section">
      <div class="total-box">
        <div class="total-row"><span>Subtotal</span><span style="font-family:monospace;font-weight:600;color:#0f172a">$${parseFloat(inv.total_price||0).toFixed(2)}</span></div>
        <div class="total-row"><span>Tax (0%)</span><span style="font-family:monospace;font-weight:600;color:#0f172a">$0.00</span></div>
        <div class="total-final"><span>TOTAL</span><span style="font-family:monospace">$${parseFloat(inv.total_price||0).toFixed(2)}</span></div>
      </div>
    </div>
    <div class="footer">
      <div><div style="font-size:12px;font-weight:700;color:#14b8a6">📦 StockPro Management System</div><div style="font-size:11px;color:#94a3b8">Thank you • Generated ${new Date().toLocaleString()}</div></div>
      <div style="font-size:10px;color:#94a3b8">v1.0.0</div>
    </div>
    ...
    <div style={{ background: dm.bg, minHeight: '100vh' }}>
        ...
        <div style={{ display:'flex', gap:3, minWidth:'max-content', padding:'6px', background:dm.card2, borderRadius:12, border:`1px solid ${dm.border}`, backdropFilter:'blur(20px)', width:'fit-content' }}>
          {[
            ...
            <button key={tab.key} onClick={()=>setActiveTab(tab.key)}
              style={{ padding: isMobile?'7px 12px':'7px 16px', borderRadius:8, fontSize: isMobile?11:12, fontWeight:600, cursor:'pointer', border:'none', fontFamily:'inherit', transition:'all 0.15s', whiteSpace:'nowrap',
      ...
      {activeTab === 'invoice' && (
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          {/* ✅ Summary Section with Time Range */}
          <div style={{ ...glass, borderRadius:18, padding: isMobile?'16px':'20px 24px' }}>
            {/* Title + Range Buttons */}
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18, flexWrap:'wrap', gap:10 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <div style={{ width:36, height:36, borderRadius:10, background:'linear-gradient(135deg,#14b8a6,#6366f1)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <i className="bi bi-bar-chart-line" style={{ color:'#fff', fontSize:15 }}></i>
                </div>
                <div>
                  <div style={{ fontWeight:700, fontSize:14, color:dm.text }}>Transaction Summary</div>
                  <div style={{ fontSize:11, color:dm.muted, marginTop:1 }}>{rangeTx.length} transactions — {rangeLabelMap[summaryRange]}</div>
                </div>
              </div>
              {/* Range Tabs */}
              <div style={{ display:'flex', background:dm.card2, borderRadius:10, padding:3, border:`1px solid ${dm.border}`, gap:2, flexWrap:'wrap' }}>
                {[
                  ...
                  <button key={r.key} onClick={()=>setSummaryRange(r.key)}
                    style={{ padding: isMobile?'5px 10px':'5px 14px', borderRadius:8, border:'none', cursor:'pointer', fontSize: isMobile?10:12, fontWeight:600, fontFamily:'inherit', transition:'all 0.15s', whiteSpace:'nowrap',
                      background: summaryRange===r.key?'linear-gradient(135deg,#14b8a6,#6366f1)':'transparent',
                      color:      summaryRange===r.key?'#fff':dm.muted,
                      boxShadow:  summaryRange===r.key?'0 4px 12px rgba(20,184,166,0.3)':'none',
                    }}>
                    {r.label}
                  </button>
                ))}
            ...
            <div style={{ display:'grid', gridTemplateColumns: isMobile?'repeat(2,1fr)':'repeat(6,1fr)', gap:10, marginBottom:14 }}>
              {[
                { label:'Transactions',  value: rangeTx.length,                     icon:'bi-receipt',           color:'#6366f1', bg:'rgba(99,102,241,0.1)',   border:'rgba(99,102,241,0.25)'   },
                { label:'Products',      value: rangeProducts,                       icon:'bi-box-seam',          color:'#14b8a6', bg:'rgba(20,184,166,0.1)',   border:'rgba(20,184,166,0.25)'   },
                { label:'IN Qty',        value: rangeQtyIn,                          icon:'bi-arrow-down-circle', color:'#10b981', bg:'rgba(16,185,129,0.1)',   border:'rgba(16,185,129,0.25)'   },
                { label:'OUT Qty',       value: rangeQtyOut,                         icon:'bi-arrow-up-circle',   color:'#ef4444', bg:'rgba(239,68,68,0.1)',    border:'rgba(239,68,68,0.25)'    },
                { label:'IN Cost',       value:`$${rangeInCost.toFixed(0)}`,         icon:'bi-cash-stack',        color:'#f59e0b', bg:'rgba(245,158,11,0.1)',   border:'rgba(245,158,11,0.25)'   },
                { label:'OUT Revenue',   value:`$${rangeOutRev.toFixed(0)}`,         icon:'bi-wallet2',           color:'#0ea5e9', bg:'rgba(14,165,233,0.1)',   border:'rgba(14,165,233,0.25)'   },
              ].map((c,i) => (
                <div key={i} style={{ background:c.bg, border:`1px solid ${c.border}`, borderRadius:12, padding:'14px 12px', textAlign:'center', transition:'all 0.2s', cursor:'default' }}
                  onMouseEnter={e=>{ e.currentTarget.style.transform='translateY(-3px)'; e.currentTarget.style.boxShadow=`0 8px 20px ${c.border}`; }}
                  onMouseLeave={e=>{ e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='none'; }}>
                  <div style={{ width:32, height:32, borderRadius:8, background:c.bg, border:`1px solid ${c.border}`, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 8px' }}>
                    <i className={`bi ${c.icon}`} style={{ fontSize:15, color:c.color }}></i>
                  </div>
                  <div style={{ fontSize: isMobile?16:20, fontWeight:800, color:c.color, lineHeight:1, marginBottom:4 }}>{c.value}</div>
                  <div style={{ fontSize:10, color:dm.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.5px' }}>{c.label}</div>
                </div>
              ))}
            </div>

            {/* Net Profit Banner */}
            <div style={{ padding:'14px 20px', borderRadius:12, background: rangeProfit>=0?'rgba(16,185,129,0.08)':'rgba(239,68,68,0.08)', border:`1px solid ${rangeProfit>=0?'rgba(16,185,129,0.25)':'rgba(239,68,68,0.25)'}`, display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10 }}>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:40, height:40, borderRadius:10, background: rangeProfit>=0?'rgba(16,185,129,0.15)':'rgba(239,68,68,0.15)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <i className={`bi bi-graph-${rangeProfit>=0?'up':'down'}-arrow`} style={{ fontSize:18, color: rangeProfit>=0?'#10b981':'#ef4444' }}></i>
                </div>
                <div>
                  <div style={{ fontWeight:700, fontSize:13, color:dm.text }}>Net Profit — {rangeLabelMap[summaryRange]}</div>
                  <div style={{ fontSize:11, color:dm.muted, marginTop:2 }}>
                    Revenue <span style={{ color:'#10b981', fontWeight:600 }}>${rangeOutRev.toFixed(2)}</span>
                    {' '}− Cost <span style={{ color:'#ef4444', fontWeight:600 }}>${rangeInCost.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              <div style={{ fontSize: isMobile?20:26, fontWeight:800, color: rangeProfit>=0?'#10b981':'#ef4444', fontFamily:'monospace' }}>
                  ...
                  <div style={{ padding:'20px 28px' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:16, padding:'14px 18px', background: darkMode?'rgba(255,255,255,0.04)':'#f8fafc', borderRadius:14, border:`1px solid ${dm.border}`, marginBottom:20 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:11, color:dm.muted, marginTop:3 }}>PRD-{String(selectedInvoice.product_id||0).padStart(3,'0')} • {new Date(selectedInvoice.created_at).toLocaleString()}</div>
                      </div>
                    </div>
                    {/* Details */}
                    ...
                    <div style={{ borderRadius:14, overflow:'hidden', border:`1px solid ${dm.border}` }}>
                      <div style={{ display:'flex', justifyContent:'space-between', padding:'11px 18px', fontSize:13, color:dm.muted, borderBottom:`1px solid ${dm.border}` }}>
                        <span>Subtotal</span><span style={{ fontFamily:'monospace', fontWeight:600, color:dm.text }}>${parseFloat(selectedInvoice.total_price||0).toFixed(2)}</span>
                      </div>
                      <div style={{ display:'flex', justifyContent:'space-between', padding:'11px 18px', fontSize:13, color:dm.muted, borderBottom:`1px solid ${dm.border}` }}>
                        <span>Tax (0%)</span><span style={{ fontFamily:'monospace', fontWeight:600, color:dm.text }}>$0.00</span>
                      </div>
                      <div style={{ display:'flex', justifyContent:'space-between', padding:'16px 18px', fontSize:17, fontWeight:800, background:'linear-gradient(135deg,#14b8a6,#6366f1)', color:'#fff' }}>
                        <span>TOTAL</span><span style={{ fontFamily:'monospace' }}>${parseFloat(selectedInvoice.total_price||0).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {filteredTx.length===0 ? (
                  ...
                  <tr key={t.id}
                    ...
                    <td style={{ padding:'10px 14px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        {(() => { const img = t.product_id?getProductImage(t.product_id):null; return (
              ...
              <tbody>
                {filteredProducts.length===0 ? (
                  ...
                  <tr key={p.id}
                    ...
                    <td style={{ padding:'10px 14px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        {(() => { const img=getProductImage(p.id); return (
                          <div style={{ width:30, height:30, borderRadius:8, overflow:'hidden', flexShrink:0, background: img?'none':'linear-gradient(135deg,#14b8a6,#6366f1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color:'#fff', border: img?'1.5px solid rgba(20,184,166,0.3)':'none' }}>
                ...
                <tbody>
                  {lowStock.map((p,i) => (
                    <tr key={p.id}
                      ...
                      <td style={{ padding:'10px 14px', color:dm.muted, fontFamily:'monospace' }}>{p.min_quantity}</td>
                      <td style={{ padding:'10px 14px' }}>
                        <span style={{ padding:'3px 9px', borderRadius:20, fontSize:11, fontWeight:700, background:'rgba(245,158,11,0.12)', color:'#f59e0b', border:'1px solid rgba(245,158,11,0.2)', fontFamily:'monospace' }}>+{p.min_quantity-p.quantity+10}</span>
                      </td>
